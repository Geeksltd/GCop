namespace GCop.Common.FixProvider.Style
{
    ...
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(EmptyOverrideMethodsCodeFixProvider)), Shared]
    public class EmptyOverrideMethodsCodeFixProvider : GCopCodeFixProvider
    {
        private string Title => "Remove unnecessary method";

        public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create("GCop410");
        ...
        private async Task<Document> RemoveMethod(Document document, MethodDeclarationSyntax method, CancellationToken cancellationToken)
        {
            ...
            try
            {
                parent = method.GetParent(typeof(ClassDeclarationSyntax));
                if(parent!= null)
                {
