namespace GCop.MSharp.FixProvider.Performance
{
    ...
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(DatabaseFindMethodCodeFixProvider)), Shared]
    public class DatabaseFindMethodCodeFixProvider : GCopCodeFixProvider
    {
        private string Title => "Use Database.Find method";
        public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create("GCop305");
        ...
        private async Task<Document> UseDatabaseCount(Document document, InvocationExpressionSyntax invocation, CancellationToken cancellationToken)
        {
            ...
            try
            {
                newInvocation = SyntaxFactory.InvocationExpression(
                    SyntaxFactory.ParseExpression("Database.Find" + genericName.TypeArgumentList),
